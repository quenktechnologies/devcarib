...
import {JobPanelFeaturesView} from './features'; 
...
export class JobPanelView  implements __wml.View {
   ...
   constructor(__context: Job) {
       ...
       this.template = (__this:__wml.Registry) => {
           ...
           return __this.node('div', <__wml.Attrs>{'class': 'ww-panel -default board-job-panel'}, [

        __this.node('div', <__wml.Attrs>{'class': 'ww-panel__body'}, [

        __this.node('div', <__wml.Attrs>{'class': 'board-job-panel-header'}, [

        __this.node('h3', <__wml.Attrs>{'class': 'board-job-panel-header__title'}, [
...
__this.node('div', <__wml.Attrs>{'class': 'board-job-panel-header__text'}, [

        __this.node('span', <__wml.Attrs>{'class': 'board-job-panel-company'}, [

        text (__context.company)
     ]),
__document.createTextNode(' - \u000a        '),
__this.node('span', <__wml.Attrs>{'class': 'board-job-panel-time'}, [

        text (timefromnow (__context.created_on))
     ])
     ]),
__this.registerView((new JobPanelFeaturesView(__context))).render()
     ])
     ])
     ]);
