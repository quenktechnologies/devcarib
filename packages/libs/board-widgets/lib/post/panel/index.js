...
var PostPanelView = /** @class */ (function () {
    function PostPanelView(__context) {
        ...
        this.template = function (__this) {
            return __this.node('div', { 'class': 'ww-panel -default board-post-panel' }, [
                __this.node('div', { 'class': 'ww-panel__body' }, [
                    __this.node('div', { 'class': 'board-post-panel-header' }, [
                        __this.node('h3', { 'class': 'board-post-panel-header__title' }, [
                            __this.node('a', { 'href': ('/posts/' + __context.id) }, [
                                text(__context.title)
                            ])
                        ]),
                        __this.node('div', { 'class': 'board-post-panel-header__text' }, [
                            __this.node('span', { 'class': 'board-post-panel-company' }, [
                                text(__context.company)
                            ]),
                            __document.createTextNode(' - \u000a        '),
                            __this.node('span', { 'class': 'board-post-panel-time' }, [
                                text(filters_1.timefromnow(__context.created_on))
                            ])
                        ]),
                        __this.registerView((new features_1.PostPanelFeaturesView(__context))).render()
                    ])
                ])
            ]);
        };
