...
import {PostPanelFeaturesView} from './features'; 
...
export class PostPanelView  implements __wml.View {

   constructor(__context: Post) {

       this.template = (__this:__wml.Registry) => {

           return __this.node('div', <__wml.Attrs>{'class': 'ww-panel -default board-post-panel'}, [

        __this.node('div', <__wml.Attrs>{'class': 'ww-panel__body'}, [

        __this.node('div', <__wml.Attrs>{'class': 'board-post-panel-header'}, [

        __this.node('h3', <__wml.Attrs>{'class': 'board-post-panel-header__title'}, [

        __this.node('a', <__wml.Attrs>{'href': ('/posts/' + __context.id)}, [

        text (__context.title)
     ])
     ]),
__this.node('div', <__wml.Attrs>{'class': 'board-post-panel-header__text'}, [

        __this.node('span', <__wml.Attrs>{'class': 'board-post-panel-company'}, [

        text (__context.company)
     ]),
__document.createTextNode(' - \u000a        '),
__this.node('span', <__wml.Attrs>{'class': 'board-post-panel-time'}, [

        text (timefromnow (__context.created_on))
     ])
     ]),
__this.registerView((new PostPanelFeaturesView(__context))).render()
     ])
     ])
     ]);
