...
exports.template = (_app) => ({ 'create': 
    ...
    'app': { 'views': { provider: tendrilShowNunjucks.show,
            options: [{ path: `packages/extras/board-views/views` }] }, 'middleware': { 'available': { public: { provider: express.static,
                ...
                urlencoded: { provider: bodyParser.urlencoded }, session: { provider: tendrilSessionMongodb.session, options: [{ session: { secret: process.env['SESSION_SECRET'], key: `boardsesssioncookie`,
                                resave: false,
