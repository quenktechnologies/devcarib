    ...
    'app': { 'views': { provider: tendrilShowNunjucks.show,
            options: [{ path: `packages/local/board-views/views` }] }, 'middleware': { 'available': { public: { provider: express.static,
                ...
                postPublic: { provider: express.static,
                    options: [`${__dirname}/../../packages/local/board-form-post/public`, { maxAge: 0 }] },
                adminPublic: { provider: express.static,
                    options: [`${__dirname}/../../packages/local/board-admin/public`, { maxAge: 0 }] }, log: { provider: tendrilMiddlewareMorgan.log, options: [process.env['MORGAN_LOG_FORMAT']] }, json: { provider: bodyParser.json },
