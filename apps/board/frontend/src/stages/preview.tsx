...
export class PreviewFormView implements FormView {
    ...
    View = () => {
        ...
        let data = stages.reduce((prev, curr)=> merge(prev, 
          useSelector(
            (state: Record<{values: Object }>) => state[curr].values 
        )),{});

        useEffect(()=> {
        let page = new JobPage({data}, []);
        let node = document.getElementById("preview-container") as HTMLElement;
