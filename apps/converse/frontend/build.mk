$(CONVERSE_FRONTEND_DIR): $(CONVERSE_FRONTEND_PUBLIC_DIR) \
                     $(CONVERSE_FRONTEND_FRONTEND_TEST_DIR)
	touch $@
...
$(CONVERSE_FRONTEND_FRONTEND_TEST_DIR): $(CONVERSE_FRONTEND_FRONTEND_TEST_BUNDLE_DIR)
	touch $@

$(CONVERSE_FRONTEND_FRONTEND_TEST_BUNDLE_DIR): $(CONVERSE_FRONTEND_FRONTEND_TEST_BUILD_DIR)
	rm -R $@ || true
	mkdir -p $@
	$(foreach test,$(shell find $(CONVERSE_FRONTEND_FRONTEND_TEST_BUILD_DIR)\
	   -name \*.js), $(BROWSERIFY) $(test) > $@/$(notdir $(basename $(test))).js)

$(CONVERSE_FRONTEND_FRONTEND_TEST_BUILD_DIR): $(CONVERSE_FRONTEND_FRONTEND_TEST_SRC_DIR)
	rm -R $@ || true
	cp -R $(CONVERSE_FRONTEND_FRONTEND_TEST_SRC_DIR) $@
	$(TSC) --project $@

$(CONVERSE_FRONTEND_FRONTEND_TEST_SRC_DIR): $(CONVERSE_FRONTEND_FRONTEND_TEST_SRC_DIR_FILES)
	touch $@
